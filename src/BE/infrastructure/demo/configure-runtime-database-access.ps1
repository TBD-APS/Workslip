param(
    ...
    [string]$RuntimeIdentityName = 'id-workslip-demo'
)

$ErrorActionPreference = 'Stop'

if ($CompanyName -ne 'mrsoftware' -or $Environment.ToLowerInvariant() -ne 'demo') {
    throw 'This script is hard-gated to the isolated mrsoftware/demo boundary.'
}

$resourceGroup = 'rg-mrsoftware-demo'
$sqlServerName = 'db-mrsoftware-demo-server'
$sqlServerFqdn = "$sqlServerName.database.windows.net"
$sqlDatabaseName = 'db-mrsoftware-demo'
$firewallRuleName = "AllowDemoRuntimeBootstrap-$([Guid]::NewGuid().ToString('N').Substring(0, 12))"
