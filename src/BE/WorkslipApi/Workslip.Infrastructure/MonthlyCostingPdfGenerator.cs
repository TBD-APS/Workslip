...
public sealed class MonthlyCostingPdfGenerator : IMonthlyHoursPdfGenerator
{
        ...
        return Document.Create(document =>
        {
            document.Page(page =>
            {
                ...
                page.Content().PaddingTop(12).Column(column =>
                {
                    ...
                    if (rows.Any(row => row.HourlyRate is null))
                    {
                        column.Item()
                            .Background(Color.FromHex("#FEF3C7"))
                            .Border(1)
                            .BorderColor(Color.FromHex("#F59E0B"))
                            .Padding(8)
                            .Text($"{rows.Count(row => row.HourlyRate is null)} registreringer mangler fakturerbar timepris og indgår ikke i beløbstotalen.")
                            .FontSize(8);
                    }
    ...
    private static void ComposeHeader(IContainer container, MyWorksheetsMonthResponse month)
    {
        container.BorderBottom(1.5f).BorderColor(Primary).PaddingBottom(8).Row(row =>
        {
            row.RelativeItem().Column(column =>
            {
                ...
                column.Item().Text($"Time- og faktureringsgrundlag · {MonthLabel(month.Year, month.Month)}").FontSize(18).Bold().FontColor(Primary);
    ...
    private static void ComposeKpis(IContainer container, IReadOnlyList<CostRow> rows, IReadOnlyList<EmployeeSummary> employees)
    {
        var totalHours = rows.Sum(row => row.Hours);
        var totalAmount = rows.Where(row => row.Amount.HasValue).Sum(row => row.Amount!.Value);
        container.Row(row =>
        {
            row.RelativeItem().Element(cell => Kpi(cell, "Timer i alt", $"{FormatHours(totalHours)} t"));
            row.ConstantItem(8);
            row.RelativeItem().Element(cell => Kpi(cell, "Fakturerbart beløb", FormatMoney(totalAmount)));
            row.ConstantItem(8);
        ...
        container.Column(column =>
        {
            ...
            column.Item().Table(table =>
            {
                ...
                foreach (var employee in employees)
                {
                    ...
                    table.Cell().Element(TableCell).AlignRight().Text(FormatMoney(employee.Rows.Where(row => row.Amount.HasValue).Sum(row => row.Amount!.Value))).Bold();
                }
                ...
                table.Cell().Element(TotalCell).AlignRight().Text(FormatMoney(rows.Where(row => row.Amount.HasValue).Sum(row => row.Amount!.Value))).Bold();
            });
        });
    }
    ...
    private static void ComposeEmployeeDetails(IContainer container, EmployeeSummary employee)
    {
        container.PaddingBottom(12).Column(column =>
        {
            column.Item().BorderBottom(1).BorderColor(Border).PaddingBottom(4).Row(row =>
            {
                ...
                row.AutoItem().Text($"{FormatHours(employee.Rows.Sum(item => item.Hours))} timer · {FormatMoney(employee.Rows.Where(item => item.Amount.HasValue).Sum(item => item.Amount!.Value))}")
            ...
            column.Item().Table(table =>
            {
                ...
                table.Header(header =>
                {
                    ...
                    header.Cell().Element(TableHeader).AlignRight().Text("Timer");
                    header.Cell().Element(TableHeader).AlignRight().Text("Timepris");
                    header.Cell().Element(TableHeader).AlignRight().Text("Beløb");
                });
                ...
                foreach (var row in employee.Rows)
                {
                    ...
                    table.Cell().Element(TableCell).AlignRight().Text(row.HourlyRate.HasValue ? FormatRate(row.HourlyRate.Value) : "—");
                    table.Cell().Element(TableCell).AlignRight().Text(row.Amount.HasValue ? FormatMoney(row.Amount.Value) : "—");
                }
            });
        });
    }
    ...
    private static CostRow[] Flatten(MyWorksheetsMonthResponse month) =>
        month.Weeks
    ...
    private static string FormatRate(decimal value) => $"{value.ToString("N2", DanishCulture)} kr.";
    private static string FormatMoney(decimal value) => $"{value.ToString("N2", DanishCulture)} kr.";
    ...
    private sealed record EmployeeSummary(Guid UserId, string Name, IReadOnlyList<CostRow> Rows);
}
