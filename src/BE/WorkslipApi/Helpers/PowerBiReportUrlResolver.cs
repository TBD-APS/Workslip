...
public static class PowerBiReportUrlResolver
{
    ...
    public static PowerBiReportUrls? Resolve(string? configuredUrl)
    {
        if (string.IsNullOrWhiteSpace(configuredUrl)
            ...
            || !TryGetReportCoordinates(uri, out var reportId, out var groupId, out var tenantId, out var pageName))
        {
            return null;
        }
        ...
        if (groupId.HasValue) parameters["groupId"] = groupId.Value.ToString("D");
        if (tenantId.HasValue) parameters["ctid"] = tenantId.Value.ToString("D");
        if (pageName is not null) parameters["pageName"] = pageName;

        var fallbackBuilder = new UriBuilder(uri) { Fragment = string.Empty };
        return new PowerBiReportUrls(
            fallbackBuilder.Uri.AbsoluteUri,
            QueryHelpers.AddQueryString(SecureEmbedBaseUrl, parameters));
    }

    private static bool TryGetReportCoordinates(Uri uri, out Guid reportId, out Guid? groupId, out Guid? tenantId, out string? pageName)
    {
        reportId = Guid.Empty;
        groupId = null;
        tenantId = null;
        ...
        if (!TryGetOptionalGuidQueryValue(uri, "ctid", out tenantId)) return false;
        ...
        if (string.Equals(uri.AbsolutePath.TrimEnd('/'), "/reportEmbed", StringComparison.OrdinalIgnoreCase))
        {
            ...
            if (!Guid.TryParse(query["reportId"].FirstOrDefault(), out reportId)
                || !TryGetOptionalGuidQueryValue(uri, "groupId", out groupId)) return false;

            var queryPageName = query["pageName"].FirstOrDefault();
            if (!string.IsNullOrWhiteSpace(queryPageName))
            {
                if (!IsSafePageName(queryPageName)) return false;
        ...
        if (segments.Length < 4
            ...
            || !Guid.TryParse(segments[3], out reportId)) return false;

        if (!string.Equals(segments[1], "me", StringComparison.OrdinalIgnoreCase))
        {
            if (!Guid.TryParse(segments[1], out var parsedGroupId)) return false;
            groupId = parsedGroupId;
        }

        if (segments.Length >= 5 && IsSafePageName(segments[4])) pageName = segments[4];
        return true;
    }
    ...
    {
        if (!value.StartsWith(ReportPagePrefix, StringComparison.Ordinal) || value.Length > 128) return false;
