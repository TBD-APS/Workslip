...
public sealed class PushNotificationProcessorTests
{
    ...
    public async Task InvalidPayload_IsFailedWithoutLoggingRawPayload()
    {
        ...
        Assert.DoesNotContain(logger.Messages, message => message.Contains("Sensitive street 1", StringComparison.Ordinal));
    }
    ...
    public async Task SuccessfulDelivery_LogsContractAndCompletes()
    {
        ...
        Assert.Equal("Body", payload.RootElement.GetProperty("options").GetProperty("body").GetString());
        Assert.Equal("/app/job/123", payload.RootElement.GetProperty("options").GetProperty("data").GetProperty("url").GetString());
        Assert.Equal($"job-{TestJobId}", payload.RootElement.GetProperty("options").GetProperty("tag").GetString());
    ...
    public async Task ExpiredSubscription_IsDisabledAndNotificationCompletes()
    {
        ...
        var subscription = CreateSubscription(notification.UserId);
        var repository = new FakeNotificationRepository(subscription);
        var sender = new FakePushSender((_, _) => new PushSenderResult(false, "provider detail", true));
        var processor = CreateProcessor(repository, sender);
    ...
    public async Task TemporaryFailure_SchedulesFirstRetryWithoutPersistingProviderDetails()
    {
        ...
        var subscription = CreateSubscription(notification.UserId);
        var repository = new FakeNotificationRepository(subscription);
        var sender = new FakePushSender((_, _) => new PushSenderResult(false, "secret provider detail", false));
        var processor = CreateProcessor(repository, sender);
        ...
        Assert.InRange(status.NextAttemptUtc, before.AddSeconds(55), DateTimeOffset.UtcNow.AddSeconds(65));
    ...
    public async Task FifthTemporaryFailure_IsMarkedFailed()
    {
        ...
        var subscription = CreateSubscription(notification.UserId);
        var repository = new FakeNotificationRepository(subscription);
        var sender = new FakePushSender((_, _) => new PushSenderResult(false, "temporary", false));
        var processor = CreateProcessor(repository, sender);
        ...
        Assert.Equal("Push delivery failed after the maximum retry count.", status.LastError);
    }
    ...
    public async Task PartialFailure_RetrySkipsSubscriptionThatAlreadySucceeded()
    {
        ...
        Assert.Equal(1, sender.Calls.Count(call => call.SubscriptionId == successful.Id));
        Assert.Equal(2, sender.Calls.Count(call => call.SubscriptionId == retrying.Id));
        Assert.Contains(notification.Id, repository.CompletedNotificationIds);
    }
    ...
    public async Task UnknownType_IsFailedWithoutSending()
    {
        ...
        Assert.Equal("Unknown notification type.", Assert.Single(repository.StatusUpdates).LastError);
        Assert.Empty(sender.Calls);
    }
    ...
    public async Task UnexpectedFailure_ReschedulesPoisonItemAndContinuesBatch()
    {
        ...
        Assert.Contains(repository.StatusUpdates, update =>
            update.NotificationId == first.Id
            && update.Status == "Pending"
            && update.LastError == "Unexpected push processing failure (InvalidOperationException).);");
        Assert.Contains(second.Id, repository.CompletedNotificationIds);
    }

    private static readonly Guid TestJobId = Guid.Parse("00000000-0000-0000-0000-000000000123");
    ...
    private static PushNotificationProcessor CreateProcessor(
        ...
        new(repository, sender, new FakeNotificationService(), logger ?? new RecordingLogger<PushNotificationProcessor>());

    private static NotificationQueueRow CreateNotification(string? payloadJson = null, int retryCount = 0) => new()
    {
        Id = Guid.NewGuid(),
        UserId = Guid.NewGuid(),
    ...
    private sealed class FakePushSender : IPushSender
    {
        ...
        public FakePushSender(Func<PushSubscriptionRow, string, PushSenderResult>? send = null)
        {
    ...
    private sealed class FakeNotificationRepository(params PushSubscriptionRow[] subscriptions) : INotificationRepository
    {
        ...
        public Task<IReadOnlyList<NotificationQueueRow>> ClaimPendingNotificationsAsync(int batchSize, CancellationToken cancellationToken) =>
            Task.FromResult(ClaimedNotifications);
        ...
        {
            StatusUpdates.Add(new StatusUpdate(id, status, retryCount, nextAttemptUtc, lastError));
