...
public sealed class PushNotificationEndpointsTests
{
    ...
    public async Task RegisterSubscriptionAsync_AllowsSuperadminActor()
    {
        ...
        var result = await PushNotificationEndpoints.RegisterSubscriptionAsync(
            request,
            currentUser,
            service,
            httpContext,
        ...
        Assert.Equal("Workslip test", service.UserAgent);
    }

    [Fact]
    public async Task RegisterSubscriptionAsync_RejectsMissingActor()
    {
        var currentUser = new TestCurrentUserContext(null, null, Roles.Superadmin);
        var service = new RecordingPushSubscriptionService();
        var request = new RegisterPushSubscriptionRequest(
            "https://push.example/missing-user",
            new PushSubscriptionKeys("p256dh", "auth"));
