...
{
    [Fact]
    public async Task UpsertAsync_notifies_when_persisted_daily_total_reaches_24_hours()
    {
        var organizationId = Guid.NewGuid();
        var userId = Guid.NewGuid();
        var jobId = Guid.NewGuid();
        var workDate = new DateOnly(2026, 8, 16);
        var repository = new DailyHoursWorksheetRepository(24m, organizationId);
        ...
        var call = Assert.Single(notifications.DailyHoursCalls);
    ...
    [Fact]
    public async Task UpsertAsync_does_not_notify_when_persisted_total_is_below_limit()
    {
        var organizationId = Guid.NewGuid();
        var userId = Guid.NewGuid();
        var jobId = Guid.NewGuid();
        var repository = new DailyHoursWorksheetRepository(23.75m, organizationId);
        var notifications = new RecordingNotificationService();
        var service = CreateService(repository, notifications, organizationId, userId, jobId);
    ...
    [Fact]
    public async Task UpsertAsync_does_not_run_notification_side_effect_when_persistence_is_rejected()
    {
        var organizationId = Guid.NewGuid();
        var userId = Guid.NewGuid();
        var jobId = Guid.NewGuid();
        var repository = new DailyHoursWorksheetRepository(24m, organizationId, rejectUpsert: true);
        var notifications = new RecordingNotificationService();
        var service = CreateService(repository, notifications, organizationId, userId, jobId);
        ...
        Assert.Empty(notifications.DailyHoursCalls);
        Assert.Equal(0, repository.DailyHoursReadCount);
    }

    [Fact]
    public async Task UpsertAsync_does_not_add_daily_lookup_when_notification_service_is_not_registered()
    {
        var organizationId = Guid.NewGuid();
        var userId = Guid.NewGuid();
        var jobId = Guid.NewGuid();
    ...
    [Fact]
    public async Task UpsertAsync_remains_successful_when_post_persistence_daily_lookup_fails()
    {
        var organizationId = Guid.NewGuid();
        var userId = Guid.NewGuid();
        var jobId = Guid.NewGuid();
        var repository = new DailyHoursWorksheetRepository(24m, organizationId, rejectDailyHoursRead: true);
        var notifications = new RecordingNotificationService();
        var service = CreateService(repository, notifications, organizationId, userId, jobId);
    ...
    [Fact]
    public async Task UpsertAsync_remains_successful_when_notification_queue_fails()
    {
        var organizationId = Guid.NewGuid();
        var userId = Guid.NewGuid();
        var jobId = Guid.NewGuid();
        var repository = new DailyHoursWorksheetRepository(24m, organizationId);
        var notifications = new RecordingNotificationService(throwOnDailyHours: true);
        var service = CreateService(repository, notifications, organizationId, userId, jobId);
    ...
    private sealed class DailyHoursWorksheetRepository(
        decimal persistedDailyHours,
        Guid organizationId,
        bool rejectUpsert = false,
        bool rejectDailyHoursRead = false) : IWorksheetRepository
    {
        ...
        public Task<decimal> GetHoursForUserDayAsync(Guid requestedOrganizationId, Guid userId, DateOnly workDate, CancellationToken cancellationToken)
        {
            ...
            DailyHoursReadCount++;
            return rejectDailyHoursRead
                ? Task.FromException<decimal>(new InvalidOperationException("daily hours unavailable"))
                : Task.FromResult(persistedDailyHours);
        }
    ...
    private sealed class RecordingNotificationService(bool throwOnDailyHours = false) : INotificationService
    {
