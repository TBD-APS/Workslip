...
public sealed class AccountingIntegrationTests
{
    ...
    public async Task IntegrationEngine_ReturnsCorrectProvider_WhenOrganizationHasSetProvider()
    {
        ...
        var providers = new Dictionary<string, IAccountingProvider>
        {
            { "mock", mockProvider },
            { "economics", economicsProvider }
        };
        var engine = new IntegrationEngine(orgRepository, providers);
    ...
    public async Task DocumentSyncService_DoesNotDuplicate_WhenDocumentAlreadyMirrored()
    {
        ...
        var engine = new IntegrationEngine(new FakeOrganizationRepository(), new Dictionary<string, IAccountingProvider> { { "mock", provider } });
        ...
        // Act
        var result = await service.SyncExternalDocumentAsync(tenantId, extId, "test.pdf", "application/pdf", "http://link", CancellationToken.None);
    ...
    private sealed class FakeOrganizationRepository : IOrganizationRepository
    {
        ...
        public Task<OrganizationRow?> GetByIdAsync(Guid id, CancellationToken ct) => Task.FromResult<OrganizationRow?>(null);
