...
3. An unauthenticated `/login` visit must not request authenticated feature chunks, invitation enrollment, `App.css`, Inter/Outfit, React Hook Form, Zod, Sonner, telemetry SDKs, React Query or generated authenticated clients before interaction.
...
10. Explicit logout and internal local-session clearing must remove React Query data before another user can authenticate in the same browser.
11. Microsoft logout must end the Microsoft browser session without showing an account picker when a current `login_hint` is available, then return to `/login`.
12. Login, dev login, user updates, push registration and current-user retry must retain their existing behaviour.
13. Application Insights, Vercel Analytics, Speed Insights and service-worker registration must not block the first render.
14. Service-worker installation must not proactively download application CSS, fonts, images or lazy chunks.
15. A route visited once online must remain available on an offline revisit under the supported PWA flow.
