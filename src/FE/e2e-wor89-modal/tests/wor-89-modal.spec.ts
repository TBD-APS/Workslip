...
function jobResponse(status: 'Rejected' | 'InReview') {
  return {
    ...
    observations: {
      taskDescription: 'Test af navigation efter fortrydt afvisning',
...
async function configureAdminApp(page: Page): Promise<Diagnostics> {
  ...
  await page.addInitScript(() => {
    localStorage.setItem('authToken', 'wor-89-modal-playwright-token');
  ...
  await page.route('**/api/**', async (route) => {
    ...
    await fulfillJson(route, { error: 'unmocked_wor_89_modal_request', method, path }, 501);
  });
...
test.describe('@wor-89-modal navigation after undo rejection', () => {
  ...
  test('can open the reopened job', async ({ page }) => {
    const diagnostics = await configureAdminApp(page);
    await undoRejection(page, diagnostics);

    await page.getByRole('dialog', { name: 'Afvisningen er fortrudt' })
      .getByRole('button', { name: 'Til sagen' })
      .click();
