...
const job = (id: string, assignedUserIds: string[]) => ({ id, assignedUsers: assignedUserIds.map((userId) => ({ id: userId })) }) as JobListItemViewModel;

describe('quick navigator job search', () => {
  it('searches jobs from ordinary field-worker input', () => {
  ...
  it('keeps non-admin quick results scoped to assigned jobs', () => {
    ...
    expect(filterQuickNavigationJobs(jobs, false, 'user-1').map((item) => item.id)).toEqual(['mine']);
