          ...
          {isDesktop ? (
            ...
            <div className="job-sort-controls-scroll" data-scroll-left={sortCanScrollLeft} data-scroll-right={sortCanScrollRight}>
              <div className="job-sort-controls" ref={sortScrollRef}>
                <button type="button" className={`sort-btn${sortBy === 'reportNumber' ? ' active' : ''}`} onClick={() => handleSort('reportNumber')}>
                  Sagsnr.{sortBy === 'reportNumber' && (sortDirection === 'asc' ? <ArrowUp size={10} /> : <ArrowDown size={10} />)}
                </button>
                <button type="button" className={`sort-btn${sortBy === 'name' ? ' active' : ''}`} onClick={() => handleSort('name')}>
                  Kundenavn{sortBy === 'name' && (sortDirection === 'asc' ? <ArrowUp size={10} /> : <ArrowDown size={10} />)}
                </button>
                <button type="button" className={`sort-btn${sortBy === 'address' ? ' active' : ''}`} onClick={() => handleSort('address')}>
                  Adresse{sortBy === 'address' && (sortDirection === 'asc' ? <ArrowUp size={10} /> : <ArrowDown size={10} />)}
                </button>
                <div className="job-sort-secondary">
                  <button type="button" className={`sort-btn${sortBy === 'updatedAt' ? ' active' : ''}`} onClick={() => handleSort('updatedAt')}>
  ...
  return (
    ...
    >
      ...
      <div className="job-card-meta">
        ...
        {job.totalHours != null && (
          <span className="meta-item meta-hours"><Timer size={14} /> {job.totalHours}</span>
        )}
        <span className="meta-item meta-updated"><Clock size={14} /> Opdateret {formatDateTimeShort(job.updatedAt)}</span>
      </div>

      <div className="job-card-footer">
        <AssignedUsers users={job.assignedUsers} />
        <span className="btn-icon" aria-label="Åbn sag"><ChevronRight size={20} /></span>
      </div>
    </div>
  );
}
...
function AssignedUsers({ users }: { users: AssignedUserResponse[] }) {
  if (users.length === 0) {
    return <span className="unassigned"><User size={12} /><span>Ikke tildelt</span></span>;
  }

  return (
    <span className="cell-comma-list">
      {users.map((user) => <span key={user.id} className="cell-comma-list-item">{user.displayName}</span>)}
    </span>
  );
}
...
function InstallationTypeTags({ types }: { types: string[] }) {
  return (
    <span className="cell-comma-list">
      {types.map((type) => <span key={type} className="cell-comma-list-item">{type}</span>)}
    </span>
  );
}
