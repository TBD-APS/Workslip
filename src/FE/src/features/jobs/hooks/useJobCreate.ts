  ...
  const createMutation = usePostApiJobs({
    mutation: {
      onSuccess: (response) => {
        ...
        // createdJobIds is optional in the published create contract, and the app is a
        // PWA whose cached assets can outlive the API revision that served them. A
        // response without the field must stay handled: treat it as a single created job.
        const createdJobIds = response.createdJobIds?.length
          ? response.createdJobIds
          : [jobId];
        const promises: Promise<unknown>[] = [];

        // Atomic linking is part of the current create contract. Keep a single-job
        // fallback only for a response that predates it.
