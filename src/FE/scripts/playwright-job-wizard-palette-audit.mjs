...
try {
  for (const testCase of cases) {
    ...
    try {
      ...
      await choice.click();
      await assertBackground(choice, palette.petrol, `${testCase.name}: selected installation type must be petrol.`);
      ...
      await workKind.locator('input[type="radio"]').check();
      ...
      await assertBackground(workKind, palette.petrol, `${testCase.name}: selected work kind must be petrol.`);
      ...
      await assertBackground(irrelevant, palette.petrol, `${testCase.name}: selected Irrelevant toggle must be petrol.`);
      ...
      const completedFg = await css(completed, 'color');
      assert.equal(completedFg, palette.petrol, `${testCase.name}: completed progress icon/text must be petrol.`);
      ...
      report.push({
        ...
        verdict: 'PASS',
        palette,
        screenshots: [`${testCase.name}-selection.png`, `${testCase.name}-control-points.png`],
      });
    } finally {
      await context.close();
    }
...
async function createDraft(token, userId) {
  const response = await fetch(`${API_URL}/api/jobs/`, {
    ...
    body: JSON.stringify({
      customerId: null,
      customerSnapshot: {
        name: 'Palette QA Kunde',
        email: 'palette.qa@example.com',
        phone: '12345678',
        address: 'Testvej 1, 8000 Aarhus C',
        contactPerson: 'Palette QA',
      },
      ...
      observations: {
        reportDate: null,
        taskDescription: 'WOR-719 palette browser verification',
...
async function resolvePalette(page) {
  return page.evaluate(() => {
    ...
    const result = {
      orange: resolve(root.getPropertyValue('--primary').trim()),
      petrol: resolve(root.getPropertyValue('--color-primary').trim()),
      cream: resolve(root.getPropertyValue('--brand-cream').trim()),
    };
    probe.remove();
    return result;
  });
...
async function assertInjectedSemanticStates(page, palette, name) {
  const values = await page.evaluate(() => {
    const host = document.createElement('div');
    host.className = 'job-wizard-palette-probe';
    host.innerHTML = '<button class="calendar-picker-day selected">15</button><div class="job-wizard-tutorial-progress-step is-active"><span class="job-wizard-tutorial-progress-dot"></span></div>';
    document.body.appendChild(host);
    const calendar = getComputedStyle(host.querySelector('.calendar-picker-day.selected'));
    const tutorial = getComputedStyle(host.querySelector('.job-wizard-tutorial-progress-dot'));
    const result = { calendarBackground: calendar.backgroundColor, calendarColor: calendar.color, tutorialBackground: tutorial.backgroundColor };
    host.remove();
    return result;
  });
  assert.equal(values.calendarBackground, palette.petrol, `${name}: selected calendar day must be petrol.`);
  assert.equal(values.calendarColor, palette.cream, `${name}: selected calendar day foreground must be cream.`);
  assert.equal(values.tutorialBackground, palette.petrol, `${name}: active tutorial progress must be petrol.`);
}
...
async function assertBackground(locator, expected, message) {
  assert.equal(await css(locator, 'backgroundColor'), expected, message);
}
async function assertNotBackground(locator, forbidden, message) {
  assert.notEqual(await css(locator, 'backgroundColor'), forbidden, message);
}
async function assertForeground(locator, expected, message) {
  assert.equal(await css(locator, 'color'), expected, message);
}
