...
export function validateFocusedAdminEnvironment(env = process.env) {
  if (env.WORKSLIP_ALLOW_LOCAL_DEV_TOKEN !== 'true') {
    throw new Error('WORKSLIP_ALLOW_LOCAL_DEV_TOKEN must be exactly true for focused local browser evidence.');
  }
...
function requireValue(value, name) {
  ...
  if (!normalized) throw new Error(`${name} is required for focused local browser evidence.`);
  return normalized;
}

export async function runFocusedAdminScenario(scenarioName, viewportName) {
  if (!SUPPORTED_SCENARIOS.has(scenarioName)) {
    throw new Error(`Unsupported focused admin scenario '${scenarioName}'.`);
  }
  if (!SUPPORTED_VIEWPORTS.has(viewportName)) {
    throw new Error(`Unsupported focused admin viewport '${viewportName}'. Expected desktop or mobile.`);
  }

  const runtime = validateFocusedAdminEnvironment();
  const frontendRoot = path.resolve(path.dirname(scriptPath), '..');
  const artifactDir = path.resolve(frontendRoot, `../../artifacts/playwright-${scenarioName}-${viewportName}`);
