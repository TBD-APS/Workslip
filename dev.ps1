...
try {
    if ($switchToMain) {
        ...
        if ($null -eq $git) { throw 'git is required to use -Main.' }

        $status = @(& $git.Source -C $repoRoot status --porcelain)
        if ($LASTEXITCODE -ne 0) { throw 'Could not inspect the Git worktree before switching to main.' }
        ...
        if ($LASTEXITCODE -ne 0) { throw 'Local main cannot be fast-forwarded to origin/main. Resolve the branch state explicitly before retrying.' }
