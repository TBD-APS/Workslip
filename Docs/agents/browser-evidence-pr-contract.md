...
Keep run links and human-readable evidence notes next to that block in the PR body. `pending`, missing fields, missing inferred scenarios, page errors, or console errors keep the guard red.
...
A waiver is an exception, not a substitute for convenient evidence. Use only when browser evidence is genuinely unnecessary or unavailable and an accountable owner accepts the scoped risk:

```text
Browser-Evidence: waived
Browser-Waiver-Owner: @owner
Browser-Waiver-Reason: <specific reason and scoped residual risk>
```

The guard rejects an absent owner or a placeholder/too-short reason. The PR discussion remains the source of accountability for the exception.

## Merge-readiness behavior

The Feature Change Guard reruns when the PR body is edited. This lets implementation begin with `pending` evidence while keeping the PR fail-closed until the relevant browser scenario has actually run or an explicit waiver has been recorded.
