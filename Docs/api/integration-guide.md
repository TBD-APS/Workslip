...
The `/api/dev/token` shortcut is not a production integration mechanism. Current startup code does not actively restrict the dev route mapping by environment, so availability must not be interpreted as approval to use it.
...
GitHub Actions can run `.github/workflows/integration-tests.yml` using either the manual `base_url` input or `WORKSLIP_INTEGRATION_BASE_URL` secret.
...
- `401`: refresh/re-authenticate; do not retry forever.
- `403`: the identity is valid but lacks permission.
- `404`: do not infer whether another organization owns the resource.
- `409`: inspect `error`; resolve business conflict or idempotency misuse.
- `428`: add the required `Idempotency-Key`.
- `429`: respect throttling and back off.
- `500`: preserve correlation ID and report it; retry only when the operation is known to be safe.

## 7. Cache-aware reads

Store the `ETag` returned by cache-enabled GET endpoints and revalidate:

```http
If-None-Match: "<etag>"
```

Treat `304` as “use the cached representation”; it has no response body.

## 8. Contract verification checklist

Before integrating a new route:

1. Confirm it exists in the running build's OpenAPI document.
2. Confirm the policy in endpoint source.
