...
During job creation, an administrator may explicitly choose one independent Job per selected assignment target. The server creates the copies atomically with separate IDs, report numbers, assignment rows, worksheet ownership, requested job links, status transitions and approval lifecycles. If that option is not selected, the existing shared multi-assignment Job remains available. This choice affects only newly created Jobs and does not rewrite historical assignments.
