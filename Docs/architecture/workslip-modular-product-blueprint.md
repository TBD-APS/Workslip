...
| There is no tenant entitlement or module catalog in the domain/API. The existing billing screen is explicitly a future guide, and HelpWizard is client-side configuration. | [BillingGuide.tsx](../../src/FE/src/features/settings/routes/BillingGuide.tsx), [HelpWizard](../../src/FE/src/features/platformFlags/HelpWizard.tsx) | Build an entitlement foundation before packaging or selling modules. |
| Saassy's current product adapter is a narrow sanitized-context contract; its control plane has no product-domain persistence or direct Workslip DB path. | [ProductContextPort](../../platform/mr-saasy-control-plane/app/ProductAdapters/Contracts/ProductContextPort.php), [ADR 0014](adr/0014-mr-saasy-delivery-lifecycle-orchestration-boundary.md) | Saassy may coordinate catalog/evidence later, but Workslip needs a local, safe enforcement projection for normal product use. |
